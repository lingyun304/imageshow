...
// Scans local media and updates the media-data.json file
function scan() {
  console.log('\n🔍 开始扫描本地媒体及元数据...');
  ...
  const supportedExtensions = [...imageExtensions, ...videoExtensions, ...audioExtensions];
  ...
  folders.forEach(folder => {
    ...
    files.forEach(file => {
      ...
      const imagePath = `/media/${folder}/${file}`;
      ...
      const typeLabel = fileType === 'image' ? '图片' : (fileType === 'video' ? '视频' : '音频');
      console.log(`✅ 已扫描 [${typeLabel}]: [${category}] ${file} (含元数据: ${extractedParams.hasMetadata ? '是' : '否'})`);
    });
  });
  
  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(images, null, 2));
  console.log(`\n🎉 扫描成功！共扫描了 ${images.length} 个媒体文件，已更新前端数据库：${OUTPUT_FILE}`);
}
...
function setupSymlink(targetPath) {
  ...
  if (fs.existsSync(linkPath)) {
    try {
      ...
      console.warn('⚠️ 警告: 移除原有 media 软链接失败，尝试直接删除...');
      try {
        fs.rmSync(linkPath, { recursive: true, force: true });
      } catch (err) {
        console.error(`❌ 错误: 无法移除 ${hasPublicDir ? 'public/' : ''}media 文件夹。请手动删除对应 media 目录，然后再试。`);
        throw err;
      }
    }
  }
  ...
  console.log(`🔗 正在建立目录链接: ${hasPublicDir ? 'public/' : ''}media -> ${targetPath}`);
  ...
  try {
    ...
    if (fs.existsSync(CONFIG_FILE) && !isSwitch) {
      try {
        ...
        if (config.targetPath && fs.existsSync(config.targetPath)) {
          ...
          console.log(`📁 载入已配置的媒体目录: ${targetPath}`);
        } else {
    ...
    if (needPrompt) {
      ...
      if (isSwitch) {
        ...
        console.log('💡 首次运行或配置文件不存在，请指定您的 AI 媒体目录。');
      }
