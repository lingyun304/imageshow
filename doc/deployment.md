# PromptGallery - 编译部署与上线指南

本指南面向运维及前端开发人员，指导如何将 PromptGallery 画廊项目编译打包并部署到各类 Web 服务器或托管平台上。
...
- **图片尺寸建议:** 建议 ComfyUI 生成图片直接使用 JPG 或经过 WebP 压缩，或在前台展示时采用 WebP 格式，单个图片体积控制在 2MB 以下，从而提供最佳的页面首屏加载和瀑布流滚动流畅度度。
