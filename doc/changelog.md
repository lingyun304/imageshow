...
### 3. 精准限定生视频“参数类”选项按钮的调浅逻辑并保留“选择域”标准样式
* **需求背景**:
  精准区分页面“选择域”与“参数类”按键：顶部导航 Tab、画廊分类 Tab 及视频模式 Tab 等“选择域”保持原有标准高亮态；仅对生视频控制台中的“参数类”按键（分辨率 720P/1080P、画幅比例 16:9等、生成时长模式 对齐原视频/自定义、音效声音 自动/沿用）进行调浅处理，消除深浓纯绿块。
* **修改覆盖范围**:
  1. 修改 `src/index.css`：在 `html.fresh-mint` 浅色主题下，严格仅针对参数类按键 `.v-select-pill.active` 与 `.v-ratio-btn.active` 应用浅淡薄荷背景 (`rgba(16, 185, 129, 0.14)`)、`1.5px` 翡翠绿边框 (`#10b981`) 与深翠绿高清晰文本 (`#047857`)；在 `html.dynamic-anime` 下精准覆盖浅粉微光。
  2. 修改 `src/App.css`：恢复模式选择 Tab 等选择域的标准主色样式，保留参数类按键的浅紫微光质感。
  3. 同步更新全部配套系统文档（`doc/requirements.md`、`doc/deployment.md`、`doc/user_guide.md`）。
* **功能影响范围**:
  生视频控制台参数调节面板，实现了选择域与参数类按键样式的精准隔离与高感官审美呈现。
